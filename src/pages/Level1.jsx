...
import { doc, setDoc, arrayUnion } from "firebase/firestore"; // use setDoc 
import { db } from "../firebase";

// --- Helper functions ---
function isColorBlind(userAnswers, correctAnswers) {
  let wrongCount = 0;
  for (let i = 0; i < correctAnswers.length; i++) {
    if (userAnswers[i] !== correctAnswers[i]) wrongCount++;
  }
  return wrongCount >= 2;
}
...
const playSound = (type, isMuted) => {
  ...
  synth.triggerAttackRelease(type === 'correct' ? "C5" : "A2", "8n");
};
...
const IshiharaPlate = ({ plate, canvasSize = 300 }) => {
  ...
  const draw = useCallback((ctx) => {
    ...
    for (let i = 0; i < dotCount; i++) {
      ...
      let color;
      if (isPointInFigure(x, y)) color = getRandomFrom(figureColors);
      else color = getRandomFrom(bgColors);
...
const Confetti = () => {
  ...
  return (
    <div className="absolute inset-0 overflow-hidden pointer-events-none z-30">
      ...
      <style>{`
        @keyframes fall { to { transform: translateY(120vh); opacity: 0.6; } }
...
export default function Level1({ onGoHome, userId, age }) {
  ...
  const handleAnswer = (answer) => {
    ...
    const correctAns = plateData[currentPlateIndex].correctAnswer;
  ...
  useEffect(() => {
    if (gameState === "end" && userAnswers.length === plateData.length) {
      const correctAnswers = plateData.map(p => p.correctAnswer);
      const wrongCount = correctAnswers.filter((ca, i) => userAnswers[i] !== ca).length;
      const color_blind = isColorBlind(userAnswers, correctAnswers);

      // Debug
      console.log("[Level1 DEBUG] Session computed:", { userAnswers, correctAnswers, wrongCount, color_blind });

      const session = {
  kid: userId || "unknown",      // <-- FIX: include kid!
  age,                           // optionally track age for later analytics
  date: new Date().toISOString(),
  game: "Color Spotter",
  answers: userAnswers,
  correctAnswers,
  n_wrong: wrongCount,
  color_blind_flag: color_blind,
  status: "Completed"
};
  ...
  setDoc(
    ...
    .then(() => console.log("[Level1 DEBUG] Session saved to Firestore (setDoc):", session))
    .catch((err) => console.error("[Level1 DEBUG] Failed to save session with setDoc:", err, session));
}
    }
  }, [gameState, userAnswers /*, userId, age, etc */]);
  ...
  return (
    <div className="relative w-full min-h-screen flex flex-col items-center justify-center p-4 font-poppins overflow-hidden">
      ...
      <div className="absolute top-0 -right-1/4 w-96 h-96  rounded-full mix-blend-multiply filter blur-2xl opacity-40 animate-blob animation-delay-2000"></div>
      ...
      {gameState === 'end' && (
        <div className="relative w-full max-w-lg text-center">
          ...
          <div className="bg-white rounded-3xl shadow-2xl overflow-hidden transform transition-all animate-fade-in-up">
            ...
            <div className="p-8">
              <h1 className="text-4xl font-bold text-gray-800 mb-2">Good Game!</h1>
              <div className="flex flex-col sm:flex-row justify-center gap-4">
                <button onClick={restartGame} className="px-8 py-3 bg-blue-500 text-white font-bold rounded-xl hover:bg-blue-600 transition-all transform hover:scale-105">
                  Play Again
                </button>
                <button onClick={onGoHome} className="px-8 py-3 bg-gray-200 text-gray-800 font-bold rounded-xl hover:bg-gray-300 transition-all transform hover:scale-105">
                  Back to Home
                </button>
              </div>
            </div>
